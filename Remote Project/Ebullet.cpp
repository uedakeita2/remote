...
Ebullet::Ebullet(UNIT_ID unitID, Vector2Dbl pos)
{
	_unitID = unitID;
	_pos = pos;
	_size = { 30,30 };
	...
	VectorSwitch = { 1,1 };
	boundCnt = { 0,0 };
	_length = lpSceneMng._EaimPos - _pos;
	BulletSpeed = lpSceneMng.ESpeed;
	Init();
}


void Ebullet::Update(sharedObj plObj)
{
	if (Ebullet::DestroyProc())
	{
		return;
	}
	//  座標を更新
...
bool Ebullet::DestroyProc(void)
{
	...
	if (boundCnt.x + boundCnt.y > lpSceneMng.EMaxBoundCnt)
	{
		SetAlive(false);
	}
